...
face_cascade = cv2.CascadeClassifier(r'C:\Users\91956\miniconda3\envs\env-two\Lib\site-packages\cv2\data\haarcascade_frontalface_default.xml')
...
while True:
    ...
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)   #grayscale conversion of image
    ...
    for (x, y, w, h) in faces:
        ...
        network_input = cv2.resize(roi_gray, (48, 48))               #input to the CONV net 48x48 grayscale

        predictions = trained_model.predict(network_input[np.newaxis, :, :, np.newaxis])      #feeding the input to the network

        expression = expressions[np.argmax(predictions)]             #Network Output

        cv2.putText(frame, expression, (x, y), font, 1, (0, 0, 0), 2)      #indicates the expression
